...
class Portfolio extends Model
{
    ...
    protected $table = 'indikator_content_portfolio';

    public $rules = [
        'title'    => 'required',
        'slug'     => ['required', 'regex:/^[a-z0-9\/\:_\-\*\[\]\+\?\|]*$/i', 'unique:indikator_content_portfolio'],
    ...
    public $belongsToMany = [
        'categories' => [
            'Indikator\Content\Models\PortfolioCategories',
            'table' => 'indikator_content_portfolio_relations',
            'order' => 'name'
        ],
        'tags' => [
            'Indikator\Content\Models\Tags',
            'table' => 'indikator_content_tags_portfolio',
            'order' => 'name'
        ]
    ];
    ...
    {
        Db::table('indikator_content_portfolio_relations')->where('portfolio_id', $this->id)->delete();
        Db::table('indikator_content_tags_portfolio')->where('portfolio_id', $this->id)->delete();
    }
    ...
    public function scopeListFrontEnd($query, $options)
    {
         ...
         */
        if ($category !== null) {
            $category = PortfolioCategories::find($category);
    ...
    public static function resolveMenuItem($item, $url, $theme)
    {
        ...
        else if ($item->type == 'portfolio-list') {
            $result = [
                'items' => []
            ];
    ...
    protected static function getItemUrl($pageCode, $item, $theme)
    {
        ...
        $properties = $page->getComponentProperties('portfolioPage');
    ...
    public function getCategories()
    {
        if ($this->_categories === null) {
            $this->_categories = [];
            $list = Db::table('indikator_content_portfolio_relations')->where('portfolio_id', $this->id)->get()->all();
            
            foreach ($list as $item) {
                $category = PortfolioCategories::whereId($item->portfolio_categories_id)->first();

                if ($category->status == 1) {
                    $this->_categories[$item->portfolio_categories_id] = [
                        'name' => $category->name,
                        'slug' => $category->slug
                    ];
                }
    ...
    public function getTags()
    {
        if ($this->_tags === null) { 
            $this->_tags = [];
            $list = Db::table('indikator_content_tags_portfolio')->where('portfolio_id', $this->id)->get()->all();
