...
class News extends Model
{
    ...
    protected $table = 'indikator_content_news';
    ...
    public $belongsToMany = [
        'categories' => [
            'Indikator\Content\Models\NewsCategories',
            'table' => 'indikator_content_news_relations',
            'order' => 'name'
        ],
        'tags' => [
            'Indikator\Content\Models\Tags',
            'table' => 'indikator_content_tags_news',
            'order' => 'name'
        ]
    ];
    ...
    {
        Db::table('indikator_content_news_relations')->where('news_id', $this->id)->delete();
        Db::table('indikator_content_tags_news')->where('news_id', $this->id)->delete();
    }
    ...
    protected static function getItemUrl($pageCode, $item, $theme)
    {
        ...
        $properties = $page->getComponentProperties('newsPage');
    ...
    public function getCategories()
    {
        if ($this->_categories === null) {
            $this->_categories = [];
            $list = Db::table('indikator_content_news_relations')->where('news_id', $this->id)->get()->all();
            
            foreach ($list as $item) {
                $category = NewsCategories::whereId($item->news_categories_id)->first();

                if ($category->status == 1) {
                    $this->_categories[$item->news_categories_id] = [
                        'name' => $category->name,
                        'slug' => $category->slug
                    ];
                }
    ...
    public function getTags()
    {
        if ($this->_tags === null) { 
            $this->_tags = [];
            $list = Db::table('indikator_content_tags_news')->where('news_id', $this->id)->get()->all();
